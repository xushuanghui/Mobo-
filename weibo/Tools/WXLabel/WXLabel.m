...
#warning 设置图片的大小
                    imageDrawRect.size = CGSizeMake(self.font.pointSize * 1.4, self.font.pointSize * 1.4);
...
+ (float)getAttributedStringHeightWithString:(NSString *)text
                                  WidthValue:(float)width
                                    delegate:(id<WXLabelDelegate>)delegate
                                        font:(UIFont*)font
{
    int total_height = 0;
    ...
    NSMutableArray *ranges = [NSMutableArray array];
    NSString *regex = @"<image url = '[a-zA-Z0-9_\\.@%&\\S]*'>";
    if ([delegate respondsToSelector:@selector(imagesOfRegexStringWithWXLabel:)]) {
        regex = [delegate imagesOfRegexStringWithWXLabel:nil];
    }
